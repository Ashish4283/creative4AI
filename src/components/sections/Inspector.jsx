...
const TUTORIALS = {
    ...
    'media-converter': {
        title: 'Pro: Media Format Converter',
        duration: '15 min',
        steps: [
            {
                title: '1. Project Scope',
                content: 'Build a production-grade tool to convert video and audio files.\n\n**Flow:** Upload -> Select Format -> Convert -> Download.'
            },
            {
                title: '2. Input Interface',
                content: 'Add a **User App** node. Configure the form:\n\n1. **Field 1**: Label="Source File", Type="File Upload". Set Variable Name to `source_file`.\n2. **Field 2**: Label="Target Format", Type="Dropdown Select", Options="mp3, mp4, wav". Set Variable Name to `target_format`.'
            },
            {
                title: '3. Conversion Logic',
                content: 'Add a **Python Script** node. Connect User App to it.\n\n1. In **Requirements**, type: `moviepy`\n2. In **Code**, paste this:\n\nfrom moviepy.editor import *\n\ndef main(data):\n    # Get inputs\n    f_path = data["source_file"][0]["name"]\n    fmt = data["target_format"]\n    out_name = f"converted.{fmt}"\n\n    # Conversion Logic\n    # clip = VideoFileClip(f_path)\n    # if fmt == "mp3":\n    #    clip.audio.write_audiofile(out_name)\n    # else:\n    #    clip.write_videofile(out_name)\n\n    return {\n        "ui": {\n            "type": "media",\n            "files": [\n                { "url": f"/files/{out_name}", "mime": f"video/{fmt}" }\n            ]\n        }\n    }\n\n*Backend Note:* Ensure your server has a route like `app.get("/files/:name")` to serve the generated files.'
            },
            {
                title: '4. Delivery',
                content: 'Add an **Export / Save** node to handle the converted file.\n\nConnect the Python node to it. Now users can download their converted media!'
            }
        ]
    }
};
...
const FieldItem = ({ field, index, updateListItem, removeListItem, generateVarName }) => {
    ...
    return (
        <Reorder.Item
            ...
            className="flex flex-col gap-2 p-3 bg-slate-950/50 border border-slate-800 rounded-lg hover:border-pink-500/50 transition-colors relative group"
        >
            <div 
            ...
            <div className="pl-6">
                <div className="flex gap-2 mb-1">
                    <div className="flex-1">
                        ...
                        <input className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-xs text-slate-200 focus:border-pink-500 focus:outline-none" placeholder="Field Label" value={field.label} onChange={(e) => updateListItem('fields', index, 'label', e.target.value)} />
                    </div>
                    <div className="flex-1">
                        <label className="text-[10px] text-slate-500 uppercase font-semibold block mb-1">Variable Name</label>
                        <input 
                            className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-xs text-slate-300 font-mono focus:border-pink-500 focus:outline-none" 
                ...
                <div className="flex items-center gap-2">
                    <div className="flex-grow">
                        <select className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-xs text-slate-400 focus:border-pink-500 focus:outline-none" value={field.type} onChange={(e) => updateListItem('fields', index, 'type', e.target.value)}>
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                    {(field.type === 'text' || field.type === 'textarea' || field.type === 'number') && (
                        <input 
                            className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-[10px] text-slate-300 focus:border-pink-500 focus:outline-none col-span-2" 
                    ...
                    {field.type === 'select' && (
                        <textarea 
                            className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-[10px] text-slate-300 focus:border-pink-500 focus:outline-none col-span-2 resize-y min-h-[60px]" 
                    ...
                    {['file', 'image', 'audio', 'video', 'pdf', 'document', 'spreadsheet'].includes(field.type) && (
                        <>
                            <input 
                                className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-[10px] text-slate-300 focus:border-pink-500 focus:outline-none" 
                            ...
                            <div className="flex items-center gap-2 h-full">
                                <input 
                                    type="checkbox" 
                                    id={`multi-${index}`}
                                    className="rounded bg-slate-900 border-slate-800 text-pink-500 focus:ring-pink-500 w-3 h-3"
      ...
      <div className="flex border-b border-slate-800">
        {[
            ...
            <button
                ...
                className={`flex-1 p-3 flex justify-center items-center border-b-2 transition-colors ${
                    activeTab === tab.id 
                    ...
                    <div className="space-y-4 pt-2">
                        ...
                        {selectedNode.data.triggerType === 'manual' && (
                            <div className="space-y-3 pt-4 border-t border-slate-800 animate-in fade-in slide-in-from-top-2">
                                <div className="flex items-center justify-between">
                                    <label className="text-xs text-slate-400 uppercase font-bold tracking-wider flex items-center">Input Variables <HelpTooltip text="Data passed into the workflow when it starts." /></label>
                                ...
                                <div className="space-y-2">
                                    {(selectedNode.data.inputs || []).map((input, i) => (
                                        <div key={i} className="flex gap-2">
                                            <input className="flex-1 bg-slate-950 border border-slate-800 rounded p-1.5 text-xs text-slate-200 focus:border-blue-500 focus:outline-none" placeholder="Var Name" value={input.name} onChange={(e) => updateListItem('inputs', i, 'name', e.target.value)} />
                                            <select className="w-20 bg-slate-950 border border-slate-800 rounded p-1.5 text-xs text-slate-400 focus:border-blue-500 focus:outline-none" value={input.type} onChange={(e) => updateListItem('inputs', i, 'type', e.target.value)}><option>String</option><option>Number</option><option>Boolean</option></select>
                        ...
                        <div className="space-y-3 pt-2 border-t border-slate-800 animate-in fade-in">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-1">
                                        <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => setIsPreviewOpen(true)} title="Preview App UI"><Eye className="w-3 h-3 text-slate-400 hover:text-pink-400"/></Button>
                                        <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => addListItem('fields', { id: `f_${Date.now()}`, label: 'New Field', key: 'new_field', type: 'text', required: false })}><Plus className="w-3 h-3 mr-1 text-pink-500"/></Button>
                                    </div>
                                </div>
                                <Reorder.Group axis="y" values={selectedNode.data.fields || []} onReorder={(newOrder) => handleChange('fields', newOrder)} className="space-y-2">
                                    ...
                                    {(selectedNode.data.fields || []).length === 0 && (
                                        <div className="text-center py-4 border-2 border-dashed border-slate-800 rounded-lg">
                                            ...
                                            <Button variant="link" size="sm" className="text-pink-500 h-auto p-0 text-xs" onClick={() => addListItem('fields', { id: `f_${Date.now()}`, label: 'New Field', key: 'new_field', type: 'text' })}>Add your first field</Button>
                                        </div>
                                    )}
                                </Reorder.Group>
                                <div className="pt-3 mt-2 border-t border-slate-800">
                                    ...
                                    <div className="flex gap-2">
                                        <input 
                                            className="flex-1 bg-slate-950 border border-slate-800 rounded p-2 text-sm text-slate-200 focus:outline-none focus:border-pink-500"
