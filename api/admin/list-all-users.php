...
$payload = authenticate_request();
    ...
    try {
        $query = "SELECT 
                ...
                CASE 
                    WHEN u.role = 'super_admin' THEN 'Super Admin'
                    ELSE (SELECT GROUP_CONCAT(c.name SEPARATOR ', ') FROM cluster_members cm JOIN clusters c ON cm.cluster_id = c.id WHERE cm.user_id = u.id)
                END as cluster_name,
                CASE 
                    WHEN u.role = 'super_admin' THEN NULL
                    ELSE (SELECT GROUP_CONCAT(cm.cluster_id SEPARATOR ',') FROM cluster_members cm WHERE cm.user_id = u.id)
                END as cluster_ids
              FROM users u
